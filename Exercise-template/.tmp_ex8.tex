...
\begin{problem}
计算二重积分 $\displaystyle I=\iint_D (x+2y)\dif\sigma$，其中 $D$ 是由曲线 $y=\sqrt{x}\;(x\ge 0)$，直线 $y=1$ 及 $y$ 轴（$x=0$）围成的有界闭区域。
\end{problem}
\begin{note}
若将 $D$ 视为 $X$-型区域，则上下边界为 $y=\sqrt{x}$ 和 $y=1$，含根式。若将 $D$ 视为 $Y$-型区域（先 $x$ 后 $y$），利用 $y=\sqrt{x}\Leftrightarrow x=y^2$，所有边界均为多项式，计算简便得多。
...
\begin{tikzpicture}[scale=4.5]
    ...
    % Full dashed curves for context
    \draw[blue!40, dashed, domain=0:1.12, samples=80] plot (\x*\x, \x);
    \draw[blue!40, dashed] (-0.1,1) -- (1.3,1);

    % Filled region
    \fill[blue!12, draw=blue!60, thick]
        (0,0) -- plot[domain=0:1, samples=80] (\x*\x, \x) -- (1,1) -- (0,1) -- cycle;

    % Boundary curves on top
    \draw[blue!60, very thick, domain=0:1, samples=80] plot (\x*\x, \x);
    ...
    \node[left] at (0,1) {$1$};
    \node at (0.25,0.55) {$D$};
    \node at (0.85,0.82) {$y=1$};
    \node at (0.82,0.3) {$x=y^2$};
    \node[below left] at (0,0) {$O$};
    \fill (0,0) circle (0.025);
...
\end{note}
\begin{solution}
曲线 $y=\sqrt{x}\;(x\ge 0)$ 等价于 $x=y^2\;(y\ge 0)$。将 $D$ 视为 $Y$-型区域：
\[
D=\{(x,y)\mid 0\le y\le 1,\; 0\le x\le y^2\}.
\]
则
\begin{align*}
I &= \int_0^1\dif y\int_0^{y^2}(x+2y)\dif x
   = \int_0^1 \Bigl[\frac{x^2}{2}+2yx\Bigr]_{x=0}^{x=y^2}\dif y \\[4pt]
  &= \int_0^1 \Bigl(\frac{y^4}{2}+2y^3\Bigr)\dif y
   = \Bigl[\frac{y^5}{10}+\frac{y^4}{2}\Bigr]_0^1 \\[4pt]
  &= \frac{1}{10}+\frac{1}{2}
   = \boxed{\dfrac{3}{5}}.
\end{align*}

（若按 $X$-型计算：$0\le x\le 1,\;\sqrt{x}\le y\le 1$，会出现 $\int\sqrt{x}\dif x$ 和 $\int x\sqrt{x}\dif x$，结果相同但过程繁琐。）
\end{solution}
